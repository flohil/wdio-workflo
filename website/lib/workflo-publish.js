...
const docusaurusLibPath = path.join(__dirname, '../node_modules/docusaurus/lib')

if (
  shell.exec(
    `node ${path.join(docusaurusLibPath, 'build-files.js')} ${process.argv
      .slice(2)
      .join(' ')}`,
  ).code
) {
...
const noJekyllPath = path.join(toPath, '.nojekyll')
fs.createFile(noJekyllPath)
